...
class UndirectedGraph:
    ...
    def neighbors(self, vertex):
        return self.get_neighboring_vertices(vertex)
    ...
    def dfs_visit(self, i, dfs_timer, discovery_times, finish_times, 
        ...
        for j in neighbors:
            # If j not yet discovered, visit it
            if discovery_times[j] == None:
                dfs_tree_parent[j] = i
                self.dfs_visit(j, dfs_timer, discovery_times, finish_times,
                             dfs_tree_parent, dfs_back_edges)
            # If j is discovered but not finished, we found a back edge
            elif finish_times[j] == None:
                dfs_back_edges.append((i,j))
        # After processing all neighbors, set finish time
...
def num_connected_components(g): # g is an UndirectedGraph class
    ...
    def dfs(vertex):
        visited.add(vertex)
        for neighbor in g.get_neighboring_vertices(vertex):
...
def find_all_nodes_in_cycle(g): # g is an UndirectedGraph class
    ...
    def find_cycle_nodes(start, end, parents):
        current = start
        path = set()
        while current != end:
            path.add(current)
    ...
    for i,j in back_edges:
        cycle_nodes = find_cycle_nodes(i, j, parents)
